...
class HMTKBaseMap(object):
    ...
    imported from hmtk.
    '''
    def __init__(self, config, title, saveplot=False, filetype='png', dpi=1000):
        """
        :param dict config:
    ...
    def add_catalogue(self, catalogue, overlay=False):
        ...
        for sym in symbology:
            ...
            mag_size = 1.6 * np.min([sym[0] + 0.5, sym[1] - 0.5])
    ...
    def add_catalogueCMT(self, catalogue, overlay=False):
        '''
        ...
        #max_mag = np.max(catalogue.data['magnitude'])
        min_loc = np.where(np.array([symb[0] for symb in DEFAULT_SYMBOLOGY]) <
                           np.min(catalogue.data['magnitude_cmt']))[0][-1]
        max_loc = np.where(np.array([symb[1] for symb in DEFAULT_SYMBOLOGY]) >
                           np.max(catalogue.data['magnitude_cmt']))[0][1]
        symbology = DEFAULT_SYMBOLOGY[min_loc:max_loc]
        legend_list = []
        leg_handles = []
        for sym in symbology:
            ...
            if np.isinf(sym[0]):
                ...
                leg_str = '%5.2f <= M < %5.2f' %(sym[0], sym[1])
            idx = np.logical_and(catalogue.data['magnitude_cmt'] >= sym[0],
                                 catalogue.data['magnitude_cmt'] < sym[1])
            mag_size = 1.2 * np.min([sym[0] + 0.5, sym[1] - 0.5])
            x, y = self.m(catalogue.data['longitude_cmt'][idx],
                           catalogue.data['latitude_cmt'][idx])
            self.m.plot(x, y, sym[2], markersize=mag_size, label=leg_str)
